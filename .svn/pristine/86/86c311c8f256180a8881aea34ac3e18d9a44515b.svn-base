...
<div class="row">
  ...
  <section>
   <h1 style="margin-top: 20px; margin-bottom: 10px">게시판</h1>
    ...
    <form id="postForm" name="postForm" action="${pageContext.request.contextPath}/boards/BoardsModifyOk.bo" method="POST">

	
    <!-- summernote와 관련된 영역 -->
	
	<input type="text" placeholder ="제목" name="title" id="title" value="${board.getBoard_title()}" style="border: solid 0.1px #a9a9a9;">
	
    <div id="summernote" name="contents" onKeyup="inputLimitByteChecked( this );" data-byte="4000" style="resize: none;"></div>
...
$(document).ready(function () {
   ...
   var request = new Request("board");
   
   console.log(request);
   $('#summernote').summernote('code', '${board.getBoard_content()}');
   
/* $('#summernote').summernote(setting);
	setTimeout(function(){
		/* removeHTML(text) */
		/* $(".note-editable").text('${board.getBoard_content()}')
	}, 100);  */
})
